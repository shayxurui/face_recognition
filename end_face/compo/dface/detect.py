...
from compo.dface.models import PNet,RNet,ONet
import compo.dface.utils as utils
import compo.dface.image_tools as image_tools
...
class MtcnnDetector(object):
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            feed_imgs=feed_imgs.to(dtype=torch.float)
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
    ...
    def detect_face(self,img):
        ...
        if self.onet_detector:
            ...
            t3 = time.time() - t
            t = time.time()
            print("detect face time cost " + '{:.3f}'.format(t1+t2+t3) + '  pnet {:.3f}  rnet {:.3f}  onet {:.3f}'.format(t1, t2, t3))
